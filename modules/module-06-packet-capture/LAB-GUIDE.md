...
- Captures are generated by `generate-pcaps.py` (scapy) — edit it to change the scenarios; re-run and
  re-bake into the image.
- `port-scan.pcap` deliberately mirrors Module 02 so students see the attacker's action and the
  defender's evidence of the same event.
- The `fetch` list (`samples.txt` → baked to `/pcaps/.samples.txt`) is coordinator-curated; the
  defaults point at the Wireshark project's own GPL test captures.
