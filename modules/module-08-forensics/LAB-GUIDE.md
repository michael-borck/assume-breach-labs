...
header analysis) and `stylometry` (authorship attribution) — run from a real shell, alongside
ordinary commands (`cat`, `sed`, `wc`) for inspecting the raw evidence yourself.
...
Pick module `08`. The workstation powers on and **logs you straight in as `analyst`** on the forensic
station — a real shell, in `/home/analyst/lab`, where two cases (`emails/` and `texts/`) are waiting.
Type **`labhelp`** any time for the mission and the commands you'll need.

Everything below is a real command. Run them, inspect the output, and — because these are genuine
tools, not lab shortcuts — feel free to ask an AI assistant to explain anything you don't recognise.
...
authentication results are much harder to fake. Three messages arrived — list them:
...
Look at the raw headers, then run the trace:

```bash
sed '/^$/q' emails/legitimate.eml
mailtrace emails/legitimate.eml
```

`mailtrace` reads the headers, follows the delivery path to the **originating server** (the last
`Received:` hop), checks the sender's authentication (SPF/DKIM/DMARC), and gives a verdict.
...
> **Q3.** List the red flags `mailtrace` reported. Which single check most clearly proves the sender
> is lying about who they are? (Think about SPF: what does "the sending server is not authorised for
> that domain" mean?)

> **Q4.** A busy person sees "PayPal" and a scary subject and clicks. Name two things a normal user
> could look at — without any tools — that hint this is fake. (Tip: `cat emails/phishing.eml` to read
> the whole message, subject and body included.)

## Phase 3: Spam

```bash
mailtrace emails/spam.eml
```
...
**unknown** document whose author is disputed. The `wc -w` count shows how much text you have from
each.
...
```bash
stylometry texts unknown.txt
```

This measures the unknown document's word-length pattern and compares it to each known author using a
**chi-square** score — the *lower* the score, the *closer* the writing styles. (With no arguments,
`stylometry` uses these same defaults.)
...
- The author `stylometry` identified, the two chi-square scores, and one limitation of stylometry.

---

## Command reference

Every command in this lab is a real tool or standard shell command you run yourself:

| Task | Command |
|------|---------|
| List the email evidence | `ls emails/` |
| Show just a message's raw headers | `sed '/^$/q' emails/phishing.eml` |
| Read a whole raw message | `cat emails/phishing.eml` |
| Trace an email's origin + authentication | `mailtrace emails/phishing.eml` |
| List the writing samples | `ls texts/` |
| Count words per author sample | `wc -w texts/*.txt` |
| Identify the unknown document's author | `stylometry texts unknown.txt` |

You are `analyst` on `station`, working in `/home/analyst/lab` — run these yourself, inspect the
results, and open the raw `.eml` and author files however you like. Everything resets when you
restart the lab.
